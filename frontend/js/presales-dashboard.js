...
async function confirmEnrolment() {
  ...
  try {
    ...
    /* Insert into enrollment_requests — this is the table post-sales reads */
    const res = await fetch('https://api.stemnestacademy.co.uk/api/enrollments/request', {
      ...
      body: JSON.stringify({
        ...
        phone:       b.whatsapp   || '',
        timezone:    b.timezone   || '',
        courseName:  b.subject    || 'Coding',
        notes:       'Referred from Presales Completed Demos. Demo teacher: ' + (b.tutorName || '—') +
                     '. Grade: ' + (b.grade || '—') +
                     '. Booking ID: ' + _enrolBookingId,
        source:      'presales_completed_demo',
      }),
    });
    const data = await res.json();
    if (!data.success) throw new Error(data.error || 'Failed to create enrollment request');
